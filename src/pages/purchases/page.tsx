...
export default function PurchasesPage() {
  ...
  const { profile, isAdmin, warehouseScope } = useAuth();
  ...
  const handleStatusChange = async (
    ...
    if (status === 'cancelled' ? (!isAdmin && !isOwner) : !canDecide) return;
    ...
    <DashboardLayout title="Purchases" subtitle="Request approval to buy from a vendor, then track it through to delivery — all in one place">
      ...
      {selectedPO && (
        <PurchaseDetailModal
          ...
          canDecide={canDecide}
          canCancel={isAdmin || selectedPO.submittedBy === requesterIdentity}
        />
      )}
      {showForm && (
        <PurchaseFormModal
