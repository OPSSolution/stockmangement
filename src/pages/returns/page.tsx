  ...
  const handleSaveReturn = async (ret: ReturnRequest) => {
    ...
    if (ret.status === 'restocked' && previousStatus !== 'restocked') {
      const { error: restockError } = await restockReturnedItems(
        ret.items.map((i) => ({ productId: i.productId, warehouse: ret.warehouse, quantity: i.quantity, condition: i.condition, note: ret.inspectionNotes, binLocation: i.binLocation })),
