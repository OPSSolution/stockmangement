...
import type { TopProduct } from '../page';

type SortKey = 'revenue' | 'unitsSold' | 'onHoldRate';
...
export default function TopProductsTable({ products, loading }: TopProductsTableProps) {
  ...
  const sorted = [...products].sort((a, b) => {
    if (sortBy === 'onHoldRate') return b.onHoldRate - a.onHoldRate;
    if (sortBy === 'revenue') return b.revenue - a.revenue;
    return b.unitsSold - a.unitsSold;
  });

  const maxRevenue = products.length > 0 ? Math.max(...products.map((p) => p.revenue), 1) : 1;
    ...
    <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-5">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">Best performers by revenue and units, with current on-hold rate</p>
        </div>
        <div className="flex items-center gap-1.5 bg-gray-50 rounded-lg p-1">
          {([['revenue', 'Revenue'], ['unitsSold', 'Units Sold'], ['onHoldRate', 'On Hold Rate']] as [SortKey, string][]).map(([key, label]) => (
            <button
              key={key}
      ...
      {loading ? (
        ...
        <div className="flex items-center justify-center py-8 text-gray-400 text-sm">No product sales in this period</div>
      ) : (
        <div className="space-y-2.5">
          {sorted.map((p, idx) => {
            const revPct = Math.round((p.revenue / maxRevenue) * 100);
            const returnColor = p.onHoldRate > 5 ? 'text-red-500' : p.onHoldRate > 2.5 ? 'text-amber-600' : 'text-emerald-600';
            const returnBg = p.onHoldRate > 5 ? 'bg-red-400' : p.onHoldRate > 2.5 ? 'bg-amber-400' : 'bg-emerald-500';
                ...
                <div className="flex items-center gap-5 flex-shrink-0 text-sm">
                  ...
                  <div className="text-right w-16">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <p className={`font-bold ${returnColor}`}>{p.onHoldRate}%</p>
                    </div>
